...
const CourseDetail = () => {
  ...
  const { _id, cname, description, img, price, rating } = details[0];

  return (
    <div className="container my-5 detail-card" ref={ref}>
      <div className="card text-white bg-dark mb-3">
        ...
        <Pdf targetRef={ref} filename={`${cname}.pdf`} options={options}>
          {({ toPdf }) => (
            <div
              className="btn btn-primary pdfbtn bg-lime-400 hover:bg-lime-500 active:text-sm transition-all w-44 text-center mx-auto lg:mx-0 py-2 font-semibold rounded-lg cursor-pointer"
              onClick={toPdf}
            >
              Download PDF
